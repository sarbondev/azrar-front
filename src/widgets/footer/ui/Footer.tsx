...
function Footer() {
  ...
  return (
    <footer className="bg-[#26364A]">
      <div className="container mx-auto p-6 md:py-10">
        <div className="grid grid-cols-1 lg:grid-cols-[2fr_3fr] gap-10">
          <div className="flex flex-col gap-6">
            <Image src="/logo.svg" width={200} height={100} alt="Azrar logo" />
            ...
            <div className="flex flex-wrap gap-4">
              {icons.map((icon, index) => (
                <div
                  key={index}
                  className="border p-2 border-blue-100 text-white rounded-lg cursor-pointer hover:bg-white/10 transition-colors"
                >
                  <icon.label />
                </div>
              ))}
          ...
          <div className="flex flex-wrap justify-between gap-10">
            {links.map((item, indx) => (
              <ul key={indx} className="text-white">
                <li className="text-lg mb-4 font-semibold">{item.mainTitle}</li>
      ...
      <div className="container mx-auto p-6 md:py-10 text-white/80 items-center grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-white/10">
        <p>
